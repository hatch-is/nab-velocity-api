...
class VelocityProcessor
{
    ...
    private $connection;
    public $sessionToken;
    public static $Txn_method
        = array('verify', 'authorize', 'authorizeandcapture', 'capture',
                'adjust', 'undo', 'returnbyid', 'returnunlinked',
                'querytransactionsdetail',
                'captureall'); // array of method name to identify method request for common process
    ...
    public function __construct($applicationprofileid, $merchantprofileid,
        $workflowid, $isTestAccount, $identitytoken = null, $sessiontoken = null
    ) {
    ...
    */
    ...
    public function verify($options = array())
    {
        ...
        try {
            ...
            $xml = VelocityXmlCreator::verifyXML(
                $options
    ...
    public function authorizeAndCapture($options = array())
    {
        ...
        try {
            ...
            $xml = VelocityXmlCreator::authorizeandcaptureXML(
                $options
    ...
    */
    ...
    public function authorize($options = array())
    {
        ...
        try {
            $xml = VelocityXmlCreator::authorizeXML(
                $options
            ); // got authorize xml object.
            $xml->formatOutput = true;
            $body = $xml->saveXML();
            //echo '<xmp>'.$body.'</xmp>';
            list($error, $response) = $this->connection->post(
    ...
    */
    public function capture($options = array())
    {
        ...
        if (isset($options['amount']) && isset($options['TransactionId'])) {
            ...
            try {
                $xml = VelocityXmlCreator::captureXML(
                    $options['TransactionId'], $amount
            ...
            throw new Exception(
                VelocityMessage::$descriptions['errcapsesswfltransid']
            );
        }
    }
    ...
    */
    public function adjust($options = array())
    {
        ...
        if (isset($options['amount']) && isset($options['TransactionId'])) {
            $amount = number_format($options['amount'], 2, '.', '');
            try {
                $xml = VelocityXmlCreator::adjustXML(
                    $options['TransactionId'], $amount
            ...
            throw new Exception(
                VelocityMessage::$descriptions['erradjustsesswfltransid']
            );
        }
    }
    ...
    public function undo($options = array())
    {
        ...
        if (isset($options['TransactionId'])) {
            ...
            try {
                $xml = VelocityXmlCreator::undoXML(
                    $options['TransactionId']
                );  // got undo xml object.
                $xml->formatOutput = true;
                $body = $xml->saveXML();
            ...
            throw new Exception(
                VelocityMessage::$descriptions['errundosesswfltransid']
            );
        }
    }
    ...
    public function returnById($options = array())
    {
        ...
        if (isset($options['amount']) && isset($options['TransactionId'])) {
            $amount = number_format($options['amount'], 2, '.', '');
            try {
                $xml = VelocityXmlCreator::returnByIdXML(
                    $amount, $options['TransactionId']
            ...
            throw new Exception(
                VelocityMessage::$descriptions['errreturntranidwid']
            );
        }
    }
    ...
    public function returnUnlinked($options = array())
    {
        ...
        try {
            $xml = VelocityXmlCreator::returnUnlinkedXML(
                $options
            );  // got ReturnById xml object.
            $xml->formatOutput = true;
            $body = $xml->saveXML();
 ...
 */
    public function queryTransactionsDetail($guerytxndetail)
    {
        ...
        try {
            ...
            $Qtxnparams = new QueryTransactionsParameters(
                $guerytxndetail['querytransactionparam']
            );
            $pagingParameters = new PagingParameters(
                $guerytxndetail['PagingParameters']
            );
            $qtd = new QueryTransactionsDetail(
                $this->includeRelated, $pagingParameters, $Qtxnparams,
                $this->transactionDetailFormat
            );
            $body = (string)json_encode($qtd);
            //echo '<pre>'.$body.'</pre>'; die;
            list($error, $response) = $this->connection->post(
                'DataServices/TMS/transactionsDetail',
                array(
                    'sessiontoken' => $this->sessionToken,
                    'xml'          => $body,
    ...
    {
        if (isset($arg1) && isset($arg2) && isset($rtype)
            && ($rtype == self::$Txn_method[3] || $rtype == self::$Txn_method[4]
                || $rtype == self::$Txn_method[5]
                || $rtype == self::$Txn_method[0])
        ) {
            $path = 'Txn/'.$arg1.'/'.$arg2;

            return $path;
        } else {
            if (isset($arg1) && isset($rtype)
                && ($rtype == self::$Txn_method[2]
                    || $rtype == self::$Txn_method[6]
                    || $rtype == self::$Txn_method[7]
                    || $rtype == self::$Txn_method[1])
            ) {
                $path = 'Txn/'.$arg1;
                ...
                throw new Exception(
                    VelocityMessage::$descriptions['errcapadjpath']
                );
            }
        }
    }
        ...
        if ($error) {
            ...
            if (!empty($response)) {
                if (isset($response['BankcardTransactionResponsePro'])) {
                    ...
                    if (isset($response['BankcardCaptureResponse'])) {
 ...
 */
    private function processError($error, $response)
    {
        if (isset($response)) {
            $valerr = '';
            $reson = isset($response['ErrorResponse']['Reason'])
                ? $response['ErrorResponse']['Reason'] : null;
            $valError = is_array($response['ErrorResponse']['ValidationErrors'])
                ? $response['ErrorResponse']['ValidationErrors'] : array();
            $valerr .= $reson.'<br>';
